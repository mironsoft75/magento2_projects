...
class Save extends Banner
{
    ...
    public function execute()
    {
        ...
        if ($data = $this->getRequest()->getPostValue()) {
            ...
            /**
             * Save Banner Small Image Logo
             */
            $imageRequest = $this->getRequest()->getFiles('small_image');
            if ($imageRequest) {
                if (isset($imageRequest['name'])) {
                    $fileName = $imageRequest['name'];
                } else {
            ...
            if ($imageRequest && strlen($fileName)) {
                ...
                try {
                    ...
                    $uploader->addValidateCallback('banner_small_image', $imageAdapter, 'validateUploadFile');
                    $uploader->setAllowRenameFiles(true);
                    $uploader->setFilesDispersion(true);
                    ...
                    $data['small_image'] = \Magestore\Bannerslider\Model\Banner::BASE_MEDIA_PATH."/".$result['file'];
                } catch (\Exception $e) {
                    if ($e->getCode() == 0) {
                        $this->messageManager->addError($e->getMessage());
                    }
                ...
                if (isset($data['small_image']) && isset($data['small_image']['value'])) {
                    if (isset($data['small_image']['delete'])) {
                        $data['small_image'] = null;
                        $data['delete_image'] = true;
                    } elseif (isset($data['small_image']['value'])) {
