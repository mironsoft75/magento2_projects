...
class Gallery extends \Magento\Catalog\Block\Product\View\Gallery
{
            ...
            if ($collection->count()) {
                ...
                foreach ($_images as &$image) {
                    ...
                    $image['large_image_url'] = $this->image360ImageHelper
                        ...
                        ->getUrl();
                    if(isset($this->_smallImageUrl[$id])){
                        if(!$this->_smallImageUrl[$id]){
                            $imageUrl = $this->image360ImageHelper
                                ->init($product, 'product_page_image_small', ['width' => null, 'height' => null])
                                ->setImageFile($image['file'])
                                ->getUrl();
                            $this->_smallImageUrl[$id] = $imageUrl;
                        }
                    }
                    else{
                        $imageUrl = $this->image360ImageHelper
        ...
        if (!isset($this->renderedGalleryHtml[$id])) {
            ...
            if ($this->isCooperativeModeAllowed()) {
                ...
                if ($_images->count()) {
                    ...
                    foreach ($images as $image) {
                        $thumbnailImageUrl = $image->getMediumImageUrl();
                    ...
                    if ($image360Icon) {
                        ...
                        $image360Icon = $this->image360ImageHelper
                            ...
                            ->getUrl();
                        if ($thumbnailImageUrl) {
                            $image360Icon = $thumbnailImageUrl;
                        }
                    } else {
     ...
     * @return string
     */
    public function getSmallImageUrl($id){
        if(isset($this->_smallImageUrl[$id])){
            return $this->_smallImageUrl[$id];
        }
        return $this->getImage360IconPath();
    }
}
