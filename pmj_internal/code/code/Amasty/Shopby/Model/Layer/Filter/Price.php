...
class Price extends \Magento\CatalogSearch\Model\Layer\Filter\Price implements FromToFilterInterface
{
    ...
    public function apply(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        if (!empty($filter) && is_string($filter)) {
            ...
            //apply delta
            $values[0] = $values[0] ? ($values[0] - $includeBorders) : '';
            $values[1] = $values[1] ? ($values[1] + $includeBorders) : '';
            $copyFilter = implode('-', $values);

            //apply rate
            $values[0] = $values[0] ? $values[0] / $this->getCurrencyRate() : '';
            $values[1] = $values[1] ? $values[1] / $this->getCurrencyRate() : '';
