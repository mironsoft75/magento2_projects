...
class MakeNameOptions extends \Magento\Eav\Model\Entity\Attribute\Source\AbstractSource
{
    /**
     * @var OptionFactory
     */
    ...
    {
        $this->_options = $this->getAllUniqueMakeNames();
     ...
     * @return array
     */
    private function getAllUniqueMakeNames()
    {
        $makeNames = $this->ymmApiManagement->getMakeNames();
        $result = [];
        foreach ($makeNames as $make) {
            $result[] = ['label' => $make, 'value' => $make];
        }
        return $result;
    }
